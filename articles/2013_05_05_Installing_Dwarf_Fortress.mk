...
There are varies ways to do it. The easy one is to download the Mac version on the [download page](http://www.bay12games.com/dwarves/). The problem with that method is that Dwarf Therapist, an indispensable tool for playing Dwarf Fortress, won't work with the lates version. So The best way to play it is via wine, that won't make it any worse and the game will run fine without requiring configuration tweaking, something that you have to do even with the Mac version. By the way, if you download the Mac version, don't forget to change `PRINT_MODE: 2D` to `PRINT_MODE: STANDARD`, and `TRUETYPE: YES` to `TRUETYPE: NO`, in `data/init.txt`.
...
I had some problems with the WineBottler and, as being a developer, I prefer to use the command line tool. This tutorial assumes you know how to use a terminal for simple operations. And it will be easier if you already have homebrew installed.

A pro tip before you start: if you know how to use git, creat a git repository the first time you extract the Dwarf Fortress directory, and commit each time you merge another folders with it. Will make your life a lot easier if something goes wrong.
...
7. Modest mod. You may skip it if you like. Or you can choose another mod from [here](http://dwarffortresswiki.org/index.php/DF2012:List_of_mods), check for compatibility with tileset. From the Modest Mod ([thread](http://www.bay12forums.com/smf/index.php?topic=105871.0)) description:
...
To start game, run from the Dwarf Fortress directory `wine Dwarf\ Fortress.exe`. And then, to start Dwarf Therapist, run from its folder `wine DwarfTherapist.exe`.
